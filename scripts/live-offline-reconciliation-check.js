...
const usernameB = String(process.env.OFFLINE_ACCEPTANCE_USERNAME_B || usernameA).trim();
const passwordB = String(process.env.OFFLINE_ACCEPTANCE_PASSWORD_B || passwordA);
...
if (!baseUrl) {
    ...
    process.exitCode = 2;
} else if ((!tokenA || !tokenB) && (!usernameA || !passwordA)) {
    console.error("Provide Device A credentials/token. Device B may reuse Device A for same-tenant multi-device acceptance.");
    console.error("Token mode: OFFLINE_ACCEPTANCE_TOKEN_A [and optional OFFLINE_ACCEPTANCE_TOKEN_B]");
    ...
    try {
        ...
        assert.ok(claimsA?.tenantId, "Device A token has no tenant claim. Check that the live server is running the current authentication build and AUTH_TENANT_ID is configured.");
        assert.ok(claimsB?.tenantId, "Device B token has no tenant claim. Check that the live server is running the current authentication build and AUTH_TENANT_ID is configured.");
        ...
        console.log(JSON.stringify({
            success: true,
            tenantId,
            identities: claimsA.sub === claimsB.sub ? "same-auth-identity" : "two-auth-identities",
