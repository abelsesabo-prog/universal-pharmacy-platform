...
test("journal is tenant-scoped and immutable",()=>{const j=buildJournal({tenantId:"demo",idempotencyKey:"test-journal-001",lines:[{account:"CASH",side:"DEBIT",amount:"10"},{account:"SALES",side:"CREDIT",amount:"10"}]});assert.equal(j.tenantId,"demo");assert.equal(j.immutable,true);assert.equal(j.idempotencyKey,"test-journal-001");});
