...
export async function createSale({ tenantId, branchId, items = [], payments = [], customerId = null, cashierId = null, discount = 0 }) {
    ...
    if (!String(branchId || "").trim()) fail("Branch is required.");
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            const saleDoc = { tenantId, branchId: String(branchId).trim(), customerId: customerId ? String(customerId).trim() : null, cashierId: cashierId ? String(cashierId).trim() : null, subtotal, discount: discountValue, total, payments: payments.map(payment => ({ method: String(payment.method || "").trim().toUpperCase(), provider: payment.provider ? String(payment.provider).trim().toUpperCase() : null, amount: Number(payment.amount), currency: String(payment.currency || "TZS").trim().toUpperCase() })), status: "COMPLETED", createdAt: new Date() };
            const saleResult = await sales.insertOne(saleDoc, { session });

            for (const item of normalizedItems) {
                let remaining = item.quantity;
                const batchFilter = item.requestedBatchId ? { _id: item.requestedBatchId, tenantId, productId: item.productId, quantity: { $gt: 0 } } : { tenantId, productId: item.productId, quantity: { $gt: 0 }, expiryDate: { $gte: new Date() } };
                ...
                for (const batch of availableBatches) {
                    ...
                    await movements.insertOne({ tenantId, branchId: String(branchId).trim(), productId: item.productId, batchId: batch._id, type: "SALE", quantity: take, direction: "OUT", reference: `SALE:${saleResult.insertedId}`, unitCost: batch.costPrice ?? null, createdAt: new Date() }, { session });
                    remaining -= take;
                }
                if (remaining > 0) fail(`Insufficient stock for ${item.productName}.`, 409);
                await products.updateOne({ _id: item.productId, tenantId, stockQuantity: { $gte: item.quantity } }, { $inc: { stockQuantity: -item.quantity }, $set: { updatedAt: new Date() } }, { session });
