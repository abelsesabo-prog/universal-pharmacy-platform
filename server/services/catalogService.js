...
async function fetchJson(url) {
    ...
    if (!response.ok) { const error = new Error(`Catalog provider returned HTTP ${response.status}.`); error.statusCode = 502; throw error; }
    return response.json();
}

async function loadDisplayNames() {
    if (displayCache && Date.now() - displayCacheLoadedAt < DISPLAY_CACHE_TTL_MS) return displayCache;
    const payload = await fetchJson(DISPLAY_NAMES_URL);
    const names = Array.isArray(payload?.displayTerms?.term) ? payload.displayTerms.term : [];
    displayCache = [...new Set(names.map(value => String(value).trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b));
...
export async function searchCatalog(query, limit = 50) {
    ...
    return names.filter(name => normalize(name).startsWith(q)).slice(0, cappedLimit).map(name => ({ name }));
}
...
async function resolveDrugName(name) {
    ...
    for (const group of groups) {
        ...
        for (const concept of Array.isArray(group.conceptProperties) ? group.conceptProperties : []) {
            concepts.push({ rxcui: concept.rxcui, name: concept.name, tty, generic: tty === "SCD" || tty === "GPCK", branded: tty === "SBD" || tty === "BPCK" });
        }
    }
...
export async function resolveCatalogItem(name) {
    ...
    return { source: result.source, query: String(name).trim(), generics: result.concepts.filter(item => item.generic), brands: result.concepts.filter(item => item.branded) };
}

export async function installCatalogFamily({ tenantId, name, category = "Medicine" }) {
    const scopedTenantId = String(tenantId || "").trim();
    if (!scopedTenantId) { const error = new Error("Tenant context is required."); error.statusCode = 403; throw error; }
    ...
    if (!primaryGeneric && brandNames.length === 0) { const error = new Error("No catalog product was resolved."); error.statusCode = 404; throw error; }
    ...
    for (const brandName of namesToInstall) {
        ...
        try {
            const product = await createProduct(candidate, scopedTenantId);
            installed.push({ ...product, alreadyInstalled: false });
        } catch (error) {
            if (error.statusCode !== 409) throw error;
            installed.push({ ...candidate, tenantId: scopedTenantId, alreadyInstalled: true });
        }
    }

    return { source: "RxNorm", familyId, genericName: primaryGeneric, availableGenerics: genericNames, availableBrands: brandNames, installedCount: installed.filter(item => !item.alreadyInstalled).length, alreadyInstalledCount: installed.filter(item => item.alreadyInstalled).length, products: installed };
}
