...
export function createApp() {
    ...
    app.use((req, res) => req.path.startsWith("/api/") ? res.status(404).json({ success: false, error: "API route not found." }) : res.status(404).send("Not found"));
    app.use((error, req, res, next) => {
        if (res.headersSent) return next(error);
        const status = Number.isInteger(error.statusCode) ? error.statusCode : 500;
        if (status >= 500) console.error("Unhandled request error:", error);
        return res.status(status).json({ success: false, error: status >= 500 ? "Internal server error." : error.message });
    });
    return app;
}
